...
import { useMedicalRecords } from '@/lib/data-store'
...
const recordTypeLabels: Record<string, string> = {
  vaccination: 'Vaccination', diagnosis: 'Diagnosis', prescription: 'Prescription',
  procedure: 'Procedure', 'lab-result': 'Lab Result', note: 'Note',
}
...
export function MedicalRecordsContent() {
  ...
  const canCreate = hasPermission(profile?.role, 'medical_records', 'create')
  ...
  const typeOptions: { value: TypeFilter; label: string }[] = [
    { value: 'all', label: 'All Types' }, { value: 'vaccination', label: 'Vaccination' },
    { value: 'diagnosis', label: 'Diagnosis' }, { value: 'prescription', label: 'Prescription' },
    { value: 'procedure', label: 'Procedure' }, { value: 'lab-result', label: 'Lab Result' },
    { value: 'note', label: 'Note' },
  ]

  if (isLoading) {
    return <div className="flex items-center justify-center p-8"><div className="animate-pulse text-muted-foreground">Loading medical records...</div></div>
  }
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {canCreate && (
          <Button onClick={() => setDialogOpen(true)}><Plus className="size-4 mr-2" />Add Record</Button>
        )}
      </div>
        ...
        <CardHeader className="pb-4">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="relative">
              ...
              <Input placeholder="Search records..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 w-full sm:w-64" />
            </div>
          </div>
          <div className="flex gap-1 flex-wrap mt-2">
            {typeOptions.map((option) => (
              <button key={option.value} type="button" onClick={() => setTypeFilter(option.value === typeFilter ? 'all' : option.value)}
                className={`px-3 py-1.5 text-sm rounded-full transition-colors ${typeFilter === option.value ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:bg-muted/80'}`}
              >{option.label}</button>
            ))}
          </div>
        </CardHeader>
        <CardContent>
          {filteredRecords.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-muted-foreground">{searchQuery || typeFilter !== 'all' ? 'Try adjusting your filters' : 'Add medical records to track patient history'}</p>
            </div>
          ) : (
              ...
              {filteredRecords.map((record) => {
                ...
                return (
                  <div key={record.id} className="flex gap-4 rounded-lg border p-4">
                    <div className="flex size-12 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary"><Icon className="size-6" /></div>
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="mt-2 text-sm text-muted-foreground line-clamp-2">{record.description}</p>
                    </div>
                  </div>
                )
              })}
